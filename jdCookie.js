/**
 * 该脚本用于获取京东cookie。正确格式为: pt_key=xxx;pt_pin=xxx;（分号;不可少）
 *
 * 如何获取京东cookile?
 * 1. 浏览器登录网页手机版京东页面。https://m.jd.com/。登录后F12查看cookie
 * 2. 之后把cookie中的pt_key=xxx;pt_pin=xxx;填写到青龙面板的环境变量中。
 * 3. 注意登录页面不要关闭了，否则cookie会失效的
 *
 * **/
const $ = new Env('京东cookie');

// 判断青龙面板的环境变量里面是否有京东ck，并获取京东ck
let CookieJDs = []
...
console.log(`\n====================一共${CookieJDs.length}个京东账号Cookie=========\n`);
...
for (let i = 0; i < CookieJDs.length; i++) {
  ...
  jdCookieNode['CookieJD' + index] = CookieJDs[i].trim();
}
...
function jsonParse(str) {
  if (typeof str == "string") {
    try {
      ...
      console.log("jdCookie.js脚本中的json串解析异常。e = ",e);
      return [];
    }
  }
}
...
if (JSON.stringify(process.env).indexOf('GITHUB')>-1) {
  ...
  !(async () => {
    await notify.sendNotify('提醒', `请勿使用github action、滥用github资源会封我仓库以及账号`)
