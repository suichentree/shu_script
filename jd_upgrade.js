...
async function main_run(){
    try{
        ...
        for(let i =0;i<$.taskInfos.length;i++){
            let t = $.taskInfos[i];
            if (t.status === 1) {
                let subinfo = t.subTaskVOS;
                for (let j = 0; j < subinfo.length; j++) {
                    let a = subinfo[j];
                    console.log(`任务:${t.taskName};等待完成`);
                    if(t.taskType == 9 || t.taskType == 8){
                        //如何任务类型为8，9。表示这种任务可以完成多次。其余任务只能完成一次
                        let body = `{"actionType":"0","taskToken":"${a.taskToken}"}`
                        await executeGetRequest("beanDoTask",body)
                        //等待几秒
                        await $.wait(getRndInteger(t.waitDuration && t.waitDuration*1000 + 1000 || 6500, t.waitDuration && t.waitDuration*1000 + 2000 || 7000 ))
                    }else{
                        let body = `{"actionType":"0","taskToken":"${a.taskToken}"}`
                        await executeGetRequest("beanDoTask",body)
                    }
                ...
                console.log(`任务:${t.taskName};已完成`);
            }else{
                console.log(`任务:${t.taskName};未能完成\n${JSON.stringify(t)}`);
            }
        }
    }catch (e) {
        console.log("main_run主方法内部发生异常，e = ",e)
    }
}
...
async function getTaskList(){
    try{
        let body = `{"viewChannel":"AppHome"}`;
        let type = 'beanTaskList'
        ...
        console.log("getTaskList方法内部发生异常，e = ",e)
    }
}

//执行请求
async function executeGetRequest(type,body){
    try{
        let requestInfo = await initGetRequest(type,body)
        //执行请求
        if (requestInfo) {
        ...
        console.log("executeGetRequest方法内部发生异常，e = ",e)
    }
}
...
//处理请求响应数据
async function dealResponseData(type,responseData){
    try {
        data = JSON.parse(responseData);
    } catch (e) {
        console.log(`dealResponseData方法内部异常，解析响应数据失败，responseData = ${responseData}`);
        return;
    }
    switch (type) {
        case 'beanTaskList':
